...
class _CommitInfoState extends State<CommitInfo> with AutomaticKeepAliveClientMixin {
  ...
  Widget _buildTable() {
    ...
    _dataSource = MyDataSource(_colNames, _rows, _rowsCount, _countPerPage, _saveRow);
  ...
  void _sort<T>(Comparable<T> getField(Entity d), int columnIndex, bool ascending) {
   ...
   // print('${_rows[0].map['类别']} ${_rows[1].map['类别']}');
  }
}
...
class MyDataSource extends DataTableSource {
  ...
  MyDataSource(this._colNames, this._rows, this._rowsCount, this._countPerPage, this._callbackSave);
